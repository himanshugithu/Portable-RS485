...
void loop() {
  if (!digitalRead(BTN_UP)) {
    ...
    if (currentMenu == MAIN_MENU) {
      ...
      registerConfigSelection = (registerConfigSelection - 1 + 4) % 4;  // 4 items now
      // Update scroll position
      if (registerConfigSelection < startIndexRegister) {
        startIndexRegister = registerConfigSelection;
      }
  ...
  if (!digitalRead(BTN_DOWN)) {
    ...
    if (currentMenu == MAIN_MENU) {
      ...
      registerConfigSelection = (registerConfigSelection + 1) % 4;  // 4 items now
      // Update scroll position
      if (registerConfigSelection >= startIndexRegister + itemsPerPageRegister) {
...
void showRegisterValueSelection() {
  ...
  // Scroll indicators
  if (startIndexRegister > 0) display.drawTriangle(120, 10, 125, 5, 130, 10, SH110X_WHITE);
  if (startIndexRegister + itemsPerPageRegister < 4) display.drawTriangle(120, 54, 125, 59, 130, 54, SH110X_WHITE);
...
void showDataTypeSelection() {
  ...
  for (int i = 0; i < dataTypeLength; i++) {
    ...
    if (i == currentDataTypeIndex) display.print("> ");
    else display.print("  ");
